...
echo "scripthpath: $scriptpath"
declare -a platpaths
mapfile -t platpaths < <(find "$scriptpath/../../guest_setup" -maxdepth 1 -mindepth 0 -type d) 

echo "platpath: ${platpaths}"
for p in "${platpaths[@]}"; do
    echo "p= $p"
    platscript=$(find "$p" -maxdepth 1 -mindepth 0 -type f -name "launch_multios.sh")
