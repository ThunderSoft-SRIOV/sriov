...
* Choose any of the following installation methods
  * [Option 1] Create Ubuntu VM Image With  command line
  * [Option 2] Create Ubuntu VM Image With `virt-manager`
...
1. Execute the following command on the host
  ...
  cp -rf ubuntu-22.04-desktop-amd64+intel-iot.iso  ./sriov/scripts/guest_setup/ubuntu/
  cd  ./sriov/scripts/guest_setup/ubuntu/
  chmod -R +x ./*
...
### [Option 2] Create Ubuntu VM Image With `virt-manager`
1. Run `virt-manager` to start Ubuntu guest installation on host.
...
1. Run `virsh_install_ubuntu.sh` to start ubuntu guest installation.
  ...
  ```sh
  # on host
  cd  /home/idvhost/sriov/scripts/setup_guest/ubuntu/
...
```shell
# host start ubuntu
cd  /home/idvhost/sriov/scripts/guest_setup/ubuntu/
...
3. Copy the following files and directories from the /home/idvuser/ directory of the host to the /home/guest/ directory of the guest.
  ...
  cd  /home/idvhost/
  scp -r -P 2222 ./sriov  idvguest@localhost:/home/idvguest/
  ```
4. Run sriov_setup_kernel.sh in Ubuntu guest VM.
  ```shell
  cd   /home/idvguest/
  cp -rf ./sriov/sriov_patches  ./sriov/scripts/guest_setup/ubuntu/
...
7. Prepare and generate the install files in Ubuntu guest VM.
  ...
  # ./sriov/scripts/guest_setup/ubuntu/packages
  # ./sriov/scripts/guest_setup/ubuntu/sriov_install
  # ./sriov/scripts/guest_setup/ubuntu/sriov_build
  ```
...
2. The following are the corresponding versions after installing the program on guest
  ...
  ```shell
  cd  /home/idvguest/sriov/scripts/guest_setup/ubuntu/
...
```
  ...
  Check firmware guc version one host
...
1. **start_ubuntu.sh script help on host**
```shell
cd  /home/idvhost/sriov/scripts/guest_setup/ubuntu/
...
Options:
        ...
        --display specify guest display connectors configuration with HPD (Hot Plug Display) feature,
                ...
                sub-param: show-fps, show fps info on guest vm primary display.
...
```
2. **Launch Multiple Windows Guest VMs**
```shell
# on Host
cd  /home/idvhost/
